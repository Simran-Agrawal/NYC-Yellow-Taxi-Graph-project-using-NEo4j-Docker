...
class DataLoader:
    ...
    def load_transform_file(self, file_path):
        ...
        with self.driver.session() as session:
            ...
            """
            session.run(query, locations=list(set(trips['PULocationID']).union(set(trips['DOLocationID']))))
            
            # Create trip relationships
            query = """
            UNWIND $trips AS trip
            MATCH (p:Location {name: trip.pu})
            MATCH (d:Location {name: trip.do})
            MERGE (p)-[:CONNECTS {
                distance: trip.distance, 
                fare: trip.fare, 
                pickup_dt: datetime(trip.pickup_dt),
                dropoff_dt: datetime(trip.dropoff_dt)
            }]->(d)
            """
            session.run(query, trips=[{
                "pu": row['PULocationID'], 
                "do": row['DOLocationID'], 
